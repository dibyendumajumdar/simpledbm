...
/**
 * <p>
 * This Transaction Manager implements the ARIES algorithm, as described by
 * C.Mohan and others in: <cite><a
 * href="http://www.almaden.ibm.com/u/mohan/RJ6649Rev.pdf">ARIES: A Transaction
 * Recovery Method Supporting Fine-Granularity Locking and Partial Rollbacks
 * Using Write-Ahead Logging</a></cite>.
 * </p>
 * <p>
 * The implementation is fairly faithful to the published algorithms, except for
 * differences noted below:
 * </p>
 * <h3>Differences from ARIES</h3>
 * <p>
 * ARIES expects the Buffer Manager to maintain a dirty_pages table. We do not
 * insist on this, but assume that the Buffer Manager can generate such a list
 * when required.
 * </p>
 * <p>
 * We support multi-page redo log records; such redo log records are applied to
 * a set of pages. This allows changes to multiple pages be logged atomically -
 * a requirement for some implementations of BTree.
 * </p>
 * <p>
 * The ARIES paper mentions post commit actions but does not show how to
 * implement them. We provide an implementation of post commit actions. Post
 * commit actions are required to implement dropping of containers.
 * </p>
 * <p>
 * We also support non-transaction related log records. For instance, opening of
 * a container can be logged as a non-transaction related operation.
 * Non-transaction related records are applied unconditionally at system
 * restart.
 * </p>
 * <p>
 * We log a list of open containers with Checkpoint so that these can be
 * re-opened at system restart.
 * </p>
 * <h3>Limitations in current implementation</h3>
 * <ol>
 * <li>
 * It is assumed that only one thread may be executing a particular transaction.
 * This assumption leads to reduction in latching because there is no need to
 * synchronise access to the transaction control block (trx).</li>
 * <li>
 * We cannot handle more than one Buffer Manager at present.</li>
 * <li>
 * We do not deal with distributed transactions. This means that we do not need
 * to remember locks and re-acquire them during recovery.</li>
 * </ol>
 * 
 * <h3>Main areas of concern</h3>
 * <ol>
 * <li>
 * Correctness of the algorithm - I made several mistakes when implementing the
 * algorithm in C++. Basically, it is vital to not miss out something - each
 * step in the ARIES algorithm has been carefully thought out.</li>
 * <li>
 * Latch contention and deadlock. The initial implementation suffered from
 * deadlocks in at least three situations.</li>
 * </ol>
 * 
...
public final class TransactionManagerImpl implements TransactionManager {
    ...
    /**
     * Active transactions table. Note that to reduce latching, we mark dead
     * transactions as TRX_DEAD but leave them in the list for future garbage
     * collection. This enables us to avoid latching the list exclusively when
    ...
    /**
     * Protects modifications to the transaction table. Must be latched
     * exclusively when the transaction table needs to be modified.
     */
    final Latch latch;

    /**
     * The LSN from where restart redo will start. Used only during restart
     * recovery.
     */
    private Lsn redoLsn;

    /**
     * The LSN upto which it is known that transactions have committed; in other
     * words the LSN of the oldest known committed transaction.
     */
    // Lsn commitLsn;
     ...
     * values. The current value of the transaction id is saved during
     * Checkpoint. At restart the value is adjusted in case it is not be
     * up-to-date.
     */
     ...
     * Transactions save their post commit actions in the prepare record. At
     * restart we make a list of all actions that were logged in prepare records
     * of active transactions, and then ensure that these are executed when the
     * transactions commit.
     */
    private LinkedList<PostCommitAction> restartPendingActions;

    /**
     * List of active containers is logged in the CheckpointBegin record. This
     * list is used at system restart to re-open the containers.
     */
    private ActiveContainerInfo[] activeContainers;

    /**
     * This flag is used to signal to the CheckpointWriter thread that it should
     * shutdown.
     */
    ...
    /**
     * This sync object is used for signalling the CheckpointWriter thread.
     */
    // Object checkpointWriterSync = new Object();

    /**
     * This flag is used to indicate that there has been an error condition.
     */
    volatile boolean errored = false;

    /**
     * When lock requests are made, we specify a timeout value of 60 seconds.
     */
    ...
    public TransactionManagerImpl(Platform platform, LogManager logmgr,
            ...
            TransactionalModuleRegistry moduleRegistry, Properties p) {
        ...
        objectFactory.registerObjectFactory(TYPE_TRXPREPARE,
                new TransactionManagerImpl.TrxPrepare.TrxPrepareFactory(
                        loggableFactory));
        objectFactory.registerObjectFactory(TYPE_TRXABORT,
                new TransactionManagerImpl.TrxAbort.TrxAbortFactory());
        objectFactory.registerObjectFactory(TYPE_TRXEND,
        ...
        objectFactory.registerObjectFactory(TYPE_CHECKPOINTEND,
                new TransactionManagerImpl.CheckpointEnd.CheckpointEndFactory(
                        this));
        objectFactory.registerObjectFactory(TYPE_DUMMYCLR,
                new TransactionManagerImpl.DummyCLR.DummyCLRFactory());

        this.lockWaitTimeout = getNumericProperty(p, PROPERTY_LOCK_TIMEOUT,
                DEFAULT_LOCK_TIMEOUT);
        this.checkpointInterval = getNumericProperty(p,
                PROPERTY_CHECKPOINT_INTERVAL, DEFAULT_CHECKPOINT_INTERVAL);

        log.info(getClass(), "ctor", new MessageInstance(m_IX0019,
                PROPERTY_LOCK_TIMEOUT, lockWaitTimeout).toString());
        log.info(getClass(), "ctor", new MessageInstance(m_IX0019,
                PROPERTY_CHECKPOINT_INTERVAL, checkpointInterval).toString());
    }

    /**
     * Allocate the next transaction Id. Transaction Ids must be monotonically
     * increasing values. The current value of the transaction id is saved
     * during Checkpoint. At restart the value is adjusted in case it is not be
     * up-to-date.
     * <p>
     * Latching issues:<br>
    ...
    /**
     * Update the system transaction Id counter. This function is called during
     * recovery to update the master transaction ID. If a transaction read from
     * the Log has an ID that is greater than the current master ID, then, the
     * master transaction ID is set to the Log's trid.
     * <p>
     * Latching issues:<br>
     * The trxmgr is latched exclusively during recovery - hence no further
     * latching required.
     */
     ...
     * Transaction Manager when reading a new transaction from the LOG. When
     * called during recovery, a trid is supplied. Otherwise, trid is generated
     * by incrementing the master trid.
     * <p>
     * Latching issues:<br>
    ...
    private TransactionImpl newTransaction(TransactionId trxId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "newTransaction",
    ...
    /**
     * Remove a transaction from the list of active transactions. We avoid
     * removing the trx from the transaction table so that there is no need to
     * latch trxmgr.
     * <p>
     * Latching issues:<br>
    ...
    void deleteTransaction(TransactionImpl trx) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "deleteTransaction",
    ...
    /**
     * Begin a new transaction. There is no need to log this because the first
     * log record generated by a transaction is enough to indicate the start of
     * a transaction.
     * <p>
     * Latching issues:<br>
     * We acquire an exclusive latch because we are about to modify the
     * transaction table.
     */
    ...
    void validateLoggableHierarchy(Loggable loggable) {
        ...
        if (null != errCode) {
            exceptionHandler.errorThrow(this.getClass(),
                    "validateLoggableHierarchy", new TransactionException(
                            new MessageInstance(errCode, loggable)));
        }
    }
    ...
    Lsn doLogInsert(Loggable logrec) {
        ...
        if (log.isTraceEnabled()) {
            log.trace(this.getClass(), "doLogInsert",
    ...
    /**
     * Inserts non-transaction related log record. No latching needed.
     */
        ...
        if (operation instanceof NonTransactionRelatedOperation) {
            ...
            exceptionHandler.errorThrow(this.getClass(),
                    ...
                    new TransactionException(new MessageInstance(m_EX0009,
                            operation.getClass(),
                            NonTransactionRelatedOperation.class)));
            return null;
        }
    }

    /**
     * Generates a Checkpoint. The trxmgr is exclusively latched to prevent
     * modification of the transaction table. Implication is that new
     * transactions cannot start while a checkpoint is being taken.
     * 
     * @see #writeCheckpoint()
     */
    ...
    /**
     * Generates a Checkpoint. Checkpoint is generated using two records - a
     * begin and an end record.
     * <p>
     * CheckpointEnd record contains a list of dirty pages in the Buffer
     * Manager, and a list of all currently active transactions. The list of
     * open containers is logged in the checkpoint begin record.
     * </p>
     * <p>
     * Checkpoint records can potentially be large.
     * </p>
     * <p>
     * Limitation: A checkpoint record must fit into a single log record.
     * </p>
     * <p>
     * Restrictions: The ContainerDeleteOperation must not occur during
     * checkpoint. This is ensured by acquiring exclusive latch on trxmgr during
     * checkpoints, which prevents commits and rollbacks from executing
     * concurrently with checkpoints. Since the ContainerDeleteOperation is
     * performed during commit, it cannot occur concurrently with a checkpoint.
     * </p>
     * <p>
     * Latching: Caller must obtain exclusive latch on trxmgr prior to calling
     * this method. TODO: I think that we could reduce the duration of latching
     * by releasing the latch after constructing the records in memory. However,
     * still need to ensure that ContainerDeleteOperation cannot occur.
     * </p>
     */
    private void writeCheckpoint() {

        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "writeCheckPoint",
         ...
         * is in operation.  
         */
        CheckpointBegin checkpointBegin = new CheckpointBegin(0,
                TransactionManagerImpl.TYPE_CHECKPOINTBEGIN);
         ...
         * records. 
         */
        CheckpointEnd checkpointEnd = new CheckpointEnd(0,
                TransactionManagerImpl.TYPE_CHECKPOINTEND, this);
    ...
    private void reopenActiveContainers() {
        ...
        for (int i = 0; i < getActiveContainers().length; i++) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(this.getClass(), "reopenActiveContainers",
            ...
            if (sc == null) {
                sc = storageFactory.open(getActiveContainers()[i].name
                        .toString());
                storageManager.register(getActiveContainers()[i].containerId,
                        sc);
            } else {
                if (!sc.getName().equals(
                    ...
                    exceptionHandler.errorThrow(this.getClass(),
                            ...
                            new TransactionException(new MessageInstance(
                                    m_EX0010,
                                    getActiveContainers()[i].containerId, sc
                                            .getName(),
                                    getActiveContainers()[i].name.toString())));
                }
            }
     ...
     * <p>
     * Latching issues:<br>
     * We do not acquire any latches because trxmgr is exclusively latched
     * during recovery anyway.
     */
    private LogReader readCheckpoint() {
        Lsn scanLsn = logmgr.getCheckpointLsn();
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "readCheckpoint",
        ...
        if (logrec == null) {
            exceptionHandler.errorThrow(this.getClass(),
                    "readCheckpoint", new TransactionException(
                            new MessageInstance(m_EX0011)));
        }
    ...
    /**
     * Delete transactions that are unprepared and that have not generated any
     * undoable log records. Used during restart recovery.
     * <p>
     * Latching issues:<br>
    ...
    /**
     * Get the minimum recovery_lsn amongst all recovery_lsn values in the
     * dirty_pages list.
     * <p>
     * Latching issues:<br>
     * None because trxmgr is already latched.
    ...
    private void processContainerOpen(Loggable loggable) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "processContainerOpen",
    ...
    /**
     * Obtains the oldest LSN that may be required for undo recovery. This is
     * the start LSN of the oldest transaction currently active.
     * <p>
     * The Transaction Manager must be latched exclusively prior to calling this
     * method.
     */
    ...
    /**
     * Mohan: The first pass of the log that is made during restart recovery is
     * the analysis pass. The result from this pass are the transaction table,
     * which contains the list of transactions which were in unprepared state at
     * the time of system failure; the dirty_pages table, which contains the
     * list of pages that were potentially dirty in the buffers when the system
     * failed, and the RedoLSN, which is the location from which the redo pass
     * must start processing the log. The only log records that may be written
     * by this routine are end records for transactions that had totally rolled
     * back before system failure, but for whom end records are missing. During
     * this pass, if a log record is encountered for a page whose identity does
     * not already appear in the dirty_pages table, then an entry is made in the
     * table with the current log record's LSN as the page's recovery_lsn. The
     * transaction table is modified to track the state changes of transactions
     * and also to note the LSN of the most recent log record that would need to
     * be undone if it were determined ultimately that the transaction had to be
     * rolled back. The RedoLSN is the minimum of recovery_lsn from the dirty
     * pages table at the end of the analysis pass.
     * <p>
     * Latching issues:<br>
     * None because trxmgr is already latched.
     */
    private void restartAnalysis() {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "restartAnalysis",
    ...
    /**
     * Mohan: The second pass of the log that is made during restart recovery is
     * the redo pass. No logs are written in this pass. The redo pass starts
     * scanning the log records from the trxmgr->redo_lsn point. When a redoable
     * log record is encountered, a check is made to see if the referenced page
     * appears in the dirty_pages table. If it does and if the log record's LSN
     * is greater than or equal to the recovery_lsn of the page in the table,
     * then it is suspected that the page state might be such that the log
     * record's update might have to be redone. To resolve this suspicion, the
     * page is accessed. If the page's LSN is found to be less than the log
     * record's LSN, then the update is redone.
     * <p>
     * Latching issues:<br>
     * None because trxmgr is already latched.
     */
    private void restartRedo() {
        ...
        while (logrec != null && logrec.getLsn().compareTo(durableLsn) <= 0) {
            ...
            if (log.isTraceEnabled()) {
                log.trace(this.getClass(), "restartRedo",
            ...
            if (loggable instanceof Redoable) {
                    ...
                    if (dp != null
                        ...
                        try {
                            ...
                            if (page.getPageLsn().compareTo(logrec.getLsn()) < 0) {
                                ...
                                dp.setRecoveryLsn(new Lsn(page.getPageLsn()
                                        .getIndex(), page.getPageLsn()
                                        .getOffset() + 1));
                            }
                        } finally {
                            bab.unfix();
                ...
                //	                     * at restart.
                //	                     */
                //	                    log.warn(this.getClass(), "restartRedo", mcat
    ...
    /**
     * Get the maximum recovery_lsn amongst all recovery_lsn values in the
     * dirty_pages list.
     * <p>
     * Latching issues:<br>
     * None because trxmgr is already latched.
    ...
    /**
     * Mohan: This is the third pass in the recovery process. In this pass,
     * loser transactions are rolled back in reverse chronological order, in a
     * single sweep of the log. This is done by continually taking the maximum
     * of the LSNs of the next log record to be processed for each of the
     * yet-to-be-completely-undone loser transactions, until no loser
     * transaction remains to be undone. Note that during this pass the
     * dirty_pages table is not consulted. Also, since history is repeated
     * before the undo pass is initiated, the LSN on the page is not consulted
     * to determine whether an undo operation should be performed or not.
     * <p>
     * Latching issues:<br>
     * None because trxmgr is already latched.
     */
    private void restartUndo() {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "restartUndo",
        ...
        while (!undoLsn.isNull()) {
            ...
            if (loggable instanceof Compensation) {
                ...
                if (loggable instanceof Undoable) {
                    if (log.isTraceEnabled()) {
                        log.trace(this.getClass(), "restartUndo",
    ...
    public final void start() {
        ...
        log.info(this.getClass(), "start", new MessageInstance(
                m_IX0013).toString());
    }
    ...
    public final void shutdown() {
        ...
        //                log.error(this.getClass(), "shutdown", new MessageInstance(m_EX0014).toString(), e);
        //            }
        //        }
        log.info(this.getClass(), "shutdown", new MessageInstance(
                m_IX0015).toString());
    }
     ...
     * <p>
     * Latching issues: Not latched as it causes deadlock due to lack of
     * reentrancy in current implementation of Latch. When this is fixed, it
     * would be desirable to latch exclusively.
     */
    private final void doRestart() {
        ...
        if (log.isDebugEnabled()) {
            log.debug(this.getClass(), "restart",
    ...
    public static final class TransactionImpl implements Transaction, Storable,
        ...
        /**
         * lsn of the next log record that should be processed during rollback.
         */
        Lsn undoNextLsn = new Lsn();

        /**
         * List of locks held by the transaction. Locks are always added at the
         * top of the list to facilitate releasing in reverse order.
         */
        ...
        /**
         * Each post commit action is given a number - this helps to identify
         * which actions have been completed. It also can be used to identify
         * the actions that should be discarded upon rollback to a savepoint.
         */
        int actionId = 1;
        ...
        /**
         * Our isolation mode; for information only. It is client's
         * responsibility to handle differences in locking strategy based upon
         * isolation mode.
         */
        ...
        /**
         * Reference to transaction manager's latch - used to coordinate actions
         ...
         * For regular undo records, it is set to the record's lsn, but for
         * CLRs, it is set the lsn of the predecessor of the log record being
         * compensated.
         * <p>
         * Latching issues:<br>
        ...
        private final Lsn registerLsn(Loggable logrec, Lsn newLsn) {
            ...
            if (trxmgr.log.isTraceEnabled()) {
                trxmgr.log.trace(this.getClass(), "registerLsn",
        ...
        /**
         * Schedule a post commit action. Post commit actions are used to deal
         * with actions that should be deferred until it is certain that the
         * transaction is definitely committing. For example, dropping a
         * Container.
         * <p>
         ...
         * deleting a Storage Container, appropriate redo-only logs must be
         * generated. These logs should not be associated with any particular
         * transaction, and care should be taken to ensure that a checkpoint is
         * not in progress.
         */
        public synchronized final void schedulePostCommitAction(
            ...
            if (trxmgr.log.isTraceEnabled()) {
                trxmgr.log.trace(this.getClass(),
                        "schedulePostCommitAction",
                        "SIMPLEDBM-TRACE: Scheduling post commit action "
                                + action + " for transaction " + this);
            }
        ...
        /**
         * Creates a new Transaction object. Only to be used when reading from a
         * container.
         */
        ...
        /**
         * Acquires a lock in requested mode on behalf of the transaction, and
         * adds it to the transaction's list of locks. Note that the lock is
         * added only if it does not already exist within the list. This is
         * because a second request for the same lock is a conversion request.
         * <p>
         * When adding a new lock request, we assign a new lockPos value to the
         * lock. This is used when performing partial rollbacks, such as to a
         * savepoint. All locks that have an lockPos greater or equal to the
         * Savepoint can be released.
         * <p>
        ...
        private void doAcquireLock(Lockable lockable, LockMode mode,
            ...
            LockMode existing = getTrxmgr().lockmgr.findLock(this, lockable);
            getTrxmgr().lockmgr.acquire(this, lockable, mode, duration,
                    timeout);
            if (trxmgr.log.isTraceEnabled()) {
                trxmgr.log.trace(this.getClass(), "acquireLock",
                        "SIMPLEDBM-TRACE: Acquired lock on " + lockable
                                + " in mode " + mode + " for duration "
                                + duration);
            }
        ...
        /**
         * Releases locks held by the transaction. If a Savepoint is supplied,
         * only those locks are released that were acquired subsequent to the
         * Savepoint.
         * <p>
         * Latching issues:<br>
         * No latches required because there is no change to the persistent
         * state of the transaction.
         */
        private final void releaseLocks(SavepointImpl sp) {
            if (trxmgr.log.isDebugEnabled()) {
                trxmgr.log.debug(this.getClass(), "releaseLocks",
                        "SIMPLEDBM-DEBUG: Releasing locks for transaction "
                                + this + " upto savepoint " + sp);
            }
        ...
        /**
         * Discards PostCommitActions that were scheduled after specified
         * Savepoint.
         */
        private final void discardCommitActions(SavepointImpl sp) {
            if (trxmgr.log.isDebugEnabled()) {
                trxmgr.log.debug(this.getClass(),
        ...
        public synchronized final SavepointImpl createSavepoint(
            ...
            if (trxmgr.log.isDebugEnabled()) {
                trxmgr.log.debug(this.getClass(), "createSavepoint",
        ...
        /**
         * Prepare a transaction for commit. Note that since we do not support
         * distributed transactions, we do not record locks in the prepare
         * record. We do however log any PostCommitActions that have been
         * scheduled.
         * <p>
         * Latching issues:<br>
         * Caller must S latch trxmgr in order to avoid conflicts with
         * checkpoints.
         */
        private final void prepare() {
            if (trxmgr.log.isDebugEnabled()) {
                trxmgr.log.debug(this.getClass(), "prepare",
                        "SIMPLEDBM-DEBUG: Preparing to commit transaction "
                                + this);
            }
            ...
            if (!lastLsn.isNull() || !postCommitActions.isEmpty()) {
                TrxPrepare prepareLogRec = new TrxPrepare(0,
                        TransactionManagerImpl.TYPE_TRXPREPARE);
        ...
        private final void endTransaction() {
            ...
            TrxEnd commitLogRec = new TrxEnd(0,
                    TransactionManagerImpl.TYPE_TRXEND);
         ...
         * <ol>
         * <li>Prepare the transaction; log any PostCommitActions in the prepare
         * log record.</li>
         * <li>Write an end transaction record.</li>
         * <li>Perform PostCommitActions.</li>
         * <li>Release locks acquired by the transaction and mark transaction as
         * dead.</li>
         * </ol>
         * <p>
        ...
        public synchronized final void startNestedTopAction() {
            if (dummyCLR != null) {
                trxmgr.exceptionHandler.errorThrow(this.getClass(),
                        "startNestedTopAction", new TransactionException(
                                new MessageInstance(m_EX0016)));
            }
            dummyCLR = new DummyCLR(TransactionManagerImpl.MODULE_ID,
                    TransactionManagerImpl.TYPE_DUMMYCLR);
            dummyCLR.setUndoNextLsn(getLastLsn());
        }

        /**
         * Completes a nested top action. Will throw an exception if there isn't
         * a nested top action in scope.
         */
        public synchronized final void completeNestedTopAction() {
            if (dummyCLR == null) {
                trxmgr.exceptionHandler.errorThrow(this.getClass(),
                        "startNestedTopAction", new TransactionException(
                                new MessageInstance(m_EX0017)));
            }
        ...
        /**
         * Performs page oriented physical undo, ie, the undo is performed on
         * the same page that was modified during normal operation.
         */
        private final void performPhysicalUndo(TransactionalModule module,
                Undoable undoable) {
            PageId pageId = undoable.getPageId();
            BufferAccessBlock bab = getTrxmgr().bufmgr.fixExclusive(pageId,
                    false, -1, 0);
            try {
                Page page = bab.getPage();
         ...
         * orientated, ie, will affect only one page, then we follow steps
         * similar to physical undos, except that we allow the module to search
         * for the page. If this is not case, and the module may need to modify
         * multiple pages as part of undo, then we let the module handle all the
         * logic.
         * <p>
         * An example of a single page logical undo would be key insert and
         * delete operations in a btree that uses logical key deletes. Because
         * in such a case, it is known that the undo of an delete will not cause
         * a page split, as the key will simply be be reinstated by removing the
         * deleted mark.
         * <p>
        ...
        private final void performLogicalUndo(TransactionalModule module,
            ...
            if (undoable instanceof SinglePageLogicalUndo) {
                BufferAccessBlock bab = moduleFindAndFixPageForUndo(module,
                        undoable);
                try {
                    Page page = bab.getPage();
                    Compensation clr = moduleGenerateCompensation(module,
                            undoable);
                    clr.setUndoNextLsn(undoable.getPrevTrxLsn());
                    Lsn lsn = logInsert(page, clr);
                    moduleRedo(module, page, clr);
        ...
        /**
         * Rollback a transaction completely or upto a savepoint. To rollback
         * completely supply null as parameter.
         * <p>
         * Rollback starts at undoNextLsn. When a change is to be undone, we
         * invoke the Module that created the original log record. We also
         * expect the Module to record "redo" information in a Compensation Log
         * Record.
         * <p>
         * Latching issues:<br>
        ...
        private final void doRollbackInternal(Savepoint savepoint) {
            ...
            if (trxmgr.log.isDebugEnabled()) {
                if (delete) {
                    trxmgr.log.debug(this.getClass(), "rollback",
        ...
        private final void doAbort() {
            ...
            try {
                if (trxmgr.log.isDebugEnabled()) {
                    trxmgr.log.debug(this.getClass(), "abort",
                ...
                if (!lastLsn.isNull() || !postCommitActions.isEmpty()) {
                     ...
                     */
                    TrxAbort abortLogRec = new TrxAbort(0,
    ...
    /**
     * The SavepointImpl records the state of the transaction at the time it is
     * created. It records three things:
     * <ol>
     * <li>LSN - this marks the point beyond which changes can be undone.</li>
     * <li>lockPos - marks the position in the lock table beynd which locks can
     * be released.</li>
     * <li>actionId - marks the position of the PostCommitActions table beyond
     * which actions can be discarded.</li>
     * </ol>
     */
    ...
    static final class ActiveContainerInfo implements Storable {
        ...
        public StringBuilder appendTo(StringBuilder sb) {
            sb.append("ActiveContainerInfo(name=").append(name.toString())
                    .append(", id=").append(containerId).append(")");
            return sb;
        }
    ...
    /**
     * CheckpointBegin log record marks the start of a checkpoint. A list of
     * open containers is recorded in this record.
     */
        ...
        public void setActiveContainers(StorageContainerInfo[] storageContainers) {
            ...
            for (int i = 0; i < storageContainers.length; i++) {
                this.activeContainers[i] = new ActiveContainerInfo(
                        storageContainers[i].getName(), storageContainers[i]
                                .getContainerId());
            }
        }
    ...
    /**
     * CheckpointEnd record contains a list of dirty pages and the transaction
     * table.
     * 
     * @author Dibyendu Majumdar
     * @since 25-Aug-2005
    ...
    static public final class CheckpointEnd extends Checkpoint {
        ...
        public CheckpointEnd(int moduleId, int typeCode,
                TransactionManagerImpl trxmgr) {
            super(moduleId, typeCode);
            this.trxmgr = trxmgr;
        }
    ...
    /**
     * TrxPrepare record is used to log that a Transaction has reached prepared
     * state. PostCommitActions are recorded in the prepare log record. If
     * distributed transactions are to be supported, then the lock table should
     ...
     * certain updates of a transaction should be committed, irrespective of
     * whether the transaction subsequently commits or aborts. However, these
     * actions must still be atomic. A Nested Top Action supports this
     * requirement without having to initiate independent transactions to
     * perform such actions. A nested top action is created by the following
     * steps:
     * <ol>
     ...
     * <li>If this is a new page, then externalize changes to the page affected.
     * </li>
     ...
     * </ol>
     * <p>
     * Using the nested top action approach, if the enclosing transaction were
     * to rollback after the completion of the nested top action, then the dummy
     * CLR will ensure that the updates performed as part of the nested top
     * action are not undone. However, if a system failure were to occur before
     * the dummy CLR is written, then the incomplete nested top-action will be
     * undone since the nested top action's log records are written as undo-redo
     * as opposed to redo only. This provides the desired atomic property of
     * nested top actions.
     * <p>
     * Of course, if the nested top action can be expressed as a single redo
     * only log record, then that is preferable. Nested top actions are useful
     * when the action requires more than one log record, but must retain its
     * atomic property.
     * <p>
     * IMPORTANT: Note that the pageId of a DummyCLR must be NULL - this is true
     * by default, so don't change the pageId for a DummyCLR!
     * 
     * @author Dibyendu Majumdar
     * @since 27-Aug-2005
     */
    ...
    /**
     * Implements the background task for generating Checkpoints at regular
     * intervals.
     */
    public static class CheckpointWriter implements Runnable {
        ...
        //                	trxmgr.log.error(CheckpointWriter.class, "run",
