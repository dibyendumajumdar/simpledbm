...
public class DatabaseImpl extends BaseTransactionalModule implements Database {
    ...
    private void unregisterTableDefinition(int containerId) {
        ...
        getDictionaryCache()
                .unregisterRowType(tableDefinition.getContainerId());
        /* Remove the indexes */
        for (IndexDefinition idx : tableDefinition.getIndexes()) {
    ...
    public TableDefinition newTableDefinition(String name, int containerId,
        ...
        return typeSystemFactory.getTableDefinition(po, fieldFactory,
     ...
     * 
     * @param properties Properties for the database. For details see RSS
     *            Documentation.
     */
    ...
    public void createTable(TableDefinition tableDefinition) {
        ...
        try {
            ...
            BufferAccessBlock bab = server.getBufferManager().fixExclusive(
                    new PageId(0, 0), false,
                    server.getPageFactory().getRawPageType(), 0);
            try {
     ...
     * associated index definitions. The container name will be based upon the
     * table's container ID. This naming convention allows the definition to be
     * loaded if the container ID is known. Since SimpleDBM maintains a list of
     * open containers, it is possible to use this information to build the data
     * dictionary at system start up.
     * 
     ...
     * @param tableDefinition The Table Definition to be persisted
     */
    ...
    TableDefinition retrieveTableDefinition(int containerId) {
        ...
        try {
            ...
            table = typeSystemFactory.getTableDefinition(po, fieldFactory,
                    rowFactory, bb);
        } finally {
            sc.close();
        }
    ...
    public static final class CreateTableDefinition extends BaseLoggable
        ...
        public CreateTableDefinition(DatabaseImpl database, ByteBuffer bb) {
            ...
            table = database.typeSystemFactory.getTableDefinition(database.po,
                    database.fieldFactory, database.rowFactory, bb);
        }

        public CreateTableDefinition(int moduleId, int typeCode,
        ...
        static final class CreateTableDefinitionFactory implements
                ObjectFactory {
