...
public class Message implements Storable {
	...
	/**
	 * A Message holds together a number of items that relate to the same message.
	 * 
	 ...
	 * @param text The text that goes with the message, may contain argument placeholders
	 */
	public Message(char subsystem, char module, MessageType type, int errorCode, String text) {
		super();
		...
		this.code = errorCode;
		this.text = text;
		formattedText = "SIMPLEDBM-" + type.toText() + subsystem + module + pad(code) + ": " + text;
	}
