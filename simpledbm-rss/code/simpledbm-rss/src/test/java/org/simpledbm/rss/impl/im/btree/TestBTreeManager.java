...
public class TestBTreeManager extends BaseTestCase {
    ...
    /**
     * Scans the tree from a starting key to the eof, and then rolls back to
     * savepoint.
     */
    ...
    void doDeleteAndScanThreads(final boolean testUnique, final boolean commit,
        ...
        try {
                        ...
                        try {
                            ...
                            try {
                                ...
                                long n = lockWaitStarted
                                        .awaitNanos(TimeUnit.NANOSECONDS
            ...
            lock.lock();
            try {
                long n = deleted.awaitNanos(TimeUnit.NANOSECONDS.convert(2,
                        TimeUnit.SECONDS));
                assertFalse(n <= 0);
            } finally {
                lock.unlock();
    ...
    public void testIssue71_Insert() throws Exception {
        doInitContainer();
        doLoadXml(false, "org/simpledbm/rss/impl/im/btree/issue71_insert.xml");
        doSingleInsert(false, true, "a3", "10");
        doValidateTree("org/simpledbm/rss/impl/im/btree/testIssue71_insert.xml");
    }
    ...
    void doFindSingleValue(String k, String l, String expectedKey,
        ...
        try {
            ...
            try {
                ...
                IndexScan scan = index.openScan(trx, key, location, false);
                if (scan.fetchNext()) {
                    if (!(scan.getCurrentKey().toString().equals(expectedKey) && scan
                            .getCurrentLocation().toString()
                            .equals(expectedLoc))) {
                        fail("Found (" + scan.getCurrentKey().toString() + ", "
                                + scan.getCurrentLocation().toString()
                                + ") instead of (" + expectedKey + ","
                                + expectedLoc + ")");
                    }
                    scan.fetchCompleted(true);
                }
    ...
    boolean doFindKeyLockTest(IsolationMode isolationMode, String k, int loc,
        ...
        try {
            ...
            RowLocation location = (RowLocation) btree.indexItemFactory
                    .getNewLocation();
            location.loc = loc;
    ...
    /**
     * Reproduce Issue-26 - the calculation of free space is wrong when the key
     * is of leaf type and the node is an index node.
     */
...
//        final Platform platform = new PlatformImpl(p);
        ...
        final PageManager pageFactory = new PageManagerImpl(platform,
                objectFactory, storageManager, latchFactory, p);
        ...
        BTreeIndexManagerImpl.IndexItemFactory indexHelper = new BTreeIndexManagerImpl.IndexItemFactory(
                new StringKeyFactory(), new RowLocationFactory(), false);
        SlottedPageImpl page = (SlottedPageImpl) pageFactory.getInstance(spmgr
                .getPageType(), new PageId());
        page.latchExclusive();
        BTreeIndexManagerImpl.BTreeNode.formatPage(page, TYPE_STRINGKEYFACTORY,
                TYPE_ROWLOCATIONFACTORY, false, indexHelper.isUnique(), 0);
        BTreeNode node = new BTreeNode(po, indexHelper, page);
        for (int i = 1; i < 145; i++) {
            node.insert(i, generateKey(indexHelper,
                    "SimpleDBM needs to be totally bug free" + i, i, i, false));
        }
...
//        final Platform platform = new PlatformImpl(p);
    ...
    public static Test suite() {
        ...
        suite.addTest(new TestBTreeManager("testIssue71_Insert"));
    ...
    public static class BTreeDB {
        ...
        public BTreeDB(Platform platform, boolean create) throws Exception {
            ...
            if (largeBM) {
                ...
                properties.setProperty("bufferpool.writerSleepInterval",
                        "60000");
...
//            platform = new PlatformImpl(properties);
            ...
            lockmgr = (LockManagerImpl) lockmgrFactory.create(latchFactory,
                    properties);
            logmgr = (LogManagerImpl) logFactory.getLog(storageFactory,
                    properties);
            bufmgr = new BufferManagerImpl(platform, logmgr, pageFactory,
                    properties);
