 ...
 * @author Dibyendu Majumdar
 */
public final class NewLockManagerImpl implements LockManager {
    ...
    private static final String LOG_CLASS_NAME = NewLockManagerImpl.class.getName();

    private static final Logger log = Logger.getLogger(NewLockManagerImpl.class.getPackage().getName());
	 ...
	 * </p>
	 */
	private LockHandleImpl doAcquire(Object owner, Object target,
			LockMode mode, LockDuration duration, int timeout)
			throws LockException {

		LockHandleImpl handle = new LockHandleImpl(this, owner, target, mode,
				duration, timeout);
		LockRequest r = null;
		...
		if (log.isDebugEnabled()) {
			log.debug(LOG_CLASS_NAME, "acquire", "Lock requested by " + owner
					+ " for " + target + ", mode=" + mode + ", duration="
					+ duration);
		}
		...
		synchronized (bucket) {
			...
			if (r == null) {
				...
				if (log.isDebugEnabled()) {
					log.debug(LOG_CLASS_NAME, "acquire",
							"New request by transaction " + owner
									+ " for target " + target);
				}
				 ...
				 * existing grant mode.
				 */
				boolean can_grant = (!lockitem.waiting && mode
						.isCompatible(lockitem.grantedMode));
				...
				if (can_grant) {
					...
					converting = false;
					// return handleWait(handle, r, converting, duration, h,
					// lockitem);
				}
			} else {
				/*
				...
				if (log.isTraceEnabled()) {
					log.trace(LOG_CLASS_NAME, "acquire",
							"Lock conversion request by transaction " + owner
									+ " for target " + target);
				}
				...
				if (r.status == LockRequestStatus.CONVERTING
					...
					if (mode == r.mode) {
						 ...
						 * with granted group
						 */
						boolean can_grant = checkCompatible(lockitem, r, mode,
								handle);
						...
						if (can_grant) {
							...
							if (duration != LockDuration.INSTANT_DURATION) {
								...
								return handle
										.setStatus(r, LockStatus.GRANTABLE);
							}
						}
							...
							converting = true;
							// return handleWait(handle, r, converting,
							// duration, h, lockitem);
						}
					}
				}
			...
			if (!converting) {
				if (log.isDebugEnabled()) {
					log.debug(LOG_CLASS_NAME, "handleWait",
				...
				if (log.isDebugEnabled()) {
					log
							.debug(LOG_CLASS_NAME, "handleWait",
		...
		if (handle.timeout == -1) {
			...
			LockSupport.parkNanos(TimeUnit.NANOSECONDS.convert(handle.timeout,
					TimeUnit.SECONDS));
		}
		globalLock.sharedLock();
		waiters.remove(r.owner);
		h = target.hashCode() % hashTableSize;
		bucket = LockHashTable[h];		
		synchronized (bucket) {
			// if (converting)
			// chainUnlock(h);
			LockStatus status;
			...
			if (status == LockStatus.TIMEOUT)
				...
				throw new LockException(
						"SIMPLEDBM-ELOCK-002: Unable to acquire lock.");
	...
	boolean doReleaseInternal(LockHandle handle, ReleaseAction action,
			LockMode downgradeMode) throws LockException {
		LockRequest r = null;
		...
		if (log.isDebugEnabled()) {
			log.debug(LOG_CLASS_NAME, "release", "Request by " + owner
					+ " to release lock for " + target);
		}
		int h = target.hashCode() % hashTableSize;
		LockBucket bucket = LockHashTable[h];
		synchronized (bucket) {
			...
			if (r == lockitem.queueHead() && r == lockitem.queueTail()
			...
			if (action != ReleaseAction.DOWNGRADE) {
				...
				if (mode == r.mode) {
					if (log.isDebugEnabled()) {
						log.debug(LOG_CLASS_NAME, "release", "Downgrading " + r
								+ " to " + downgradeMode
								+ " and re-adjusting granted mode");
					}
    ...
    static final class LockHandleImpl implements LockHandle {
        ...
        private final NewLockManagerImpl lockMgr;
		...
		LockHandleImpl(NewLockManagerImpl lockMgr, Object owner, Object target, LockMode mode, LockDuration duration, int timeout) {
		...
		public final boolean release(boolean force) throws LockException {
			return lockMgr.doRelease(this, force ? NewLockManagerImpl.ReleaseAction.FORCE_RELEASE : NewLockManagerImpl.ReleaseAction.RELEASE, null);
		}

		public final void downgrade(LockMode mode) throws LockException {
			lockMgr.doRelease(this, NewLockManagerImpl.ReleaseAction.DOWNGRADE, mode);
		}
